...
const ChannelActivityChart = (props) => {
  ...
  // console.log('props?.data?.chartData ------------>', props?.data?.chartData)
  ...
  useEffect(() => {
    ...
    const chartDataModified = chartData.map(x => ({
      ...
      tranTypeDesc: x.srType?.description ? x.srType?.description : x.statusDesc
        ?.description || '',
    ...
    // console.log("groupBymode................>", groupBymode)
    ...
    for (const d of totArr) {
      ...
      if (['Assigned'].includes(d.name)) {
        obj.val = (obj.val ?? 0) + d.value
        obj.name = 'Assigned'
        obj.channel = d.channel
      }
      if (['Cancelled'].includes(d.name)) {
        obj.val = (obj.val ?? 0) + d.value
        obj.name = 'Cancelled'
        obj.channel = d.channel
      }
