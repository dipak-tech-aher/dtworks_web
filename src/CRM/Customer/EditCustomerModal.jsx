...
// Ramesh Made Changes on 12April24 - Helpesk creation new register customer popup issue fixing 
...
const EditCustomerModal = (props) => {
    ...
    const { customerDetails, isEditCustomerDetailsOpen, type } = props?.data
    const { setIsEditCustomerDetailsOpen, setCustomerDetails, pageRefresh,setConsumerNo } = props?.handlers
        ...
        get(properties.MASTER_API + '/lookup?searchParam=code_type&valueParam=COUNTRY,CUSTOMER_ID_TYPE,CONTACT_TYPE,GENDER,CONTACT_PREFERENCE,CUSTOMER_CLASS,MARITAL_STATUS')
            .then((response) => {
                if (response.data) {
                    ...
                    const prefix = response.data?.COUNTRY.map((e) => {
                        return {
                ...
                .then((resp) => {
                    if (resp.data) {
                        if (resp.status === 200) {
                            ...
                            // Ramesh Made Changes on 12April24 - Helpesk creation new register customer popup issue fixing 
                            setConsumerNo( resp?.data?.customerNo)
    ...
    return (
        <Modal show={isEditCustomerDetailsOpen} aria-labelledby="contained-modal-title-vcenter" centered contentLabel="" style={customStyle}  dialogClassName="cust-lg-modal">
    {/*  Ramesh Made Changes on 12April24 - Helpesk creation new register customer popup issue fixing  */}
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            {/* Ramesh Made Changes on 12April24 - Based Type i wil changed popup title */}
                            {type === 'CREATE' ? <h4 className="modal-title" id="myCenterModalLabel">Create Customer</h4> : <h4 className="modal-title" id="myCenterModalLabel">Edit - {props?.data?.customerDetails?.firstName + " " + props?.data?.customerDetails?.lastName} - {customerData?.customerNo} </h4>}
                        ...
                        <div className="modal-body">
                                    ...
                                    <div className="col-lg-3 col-md-12 col-xs-12">
                                        <>
                                            <div className="text-center mt-3">
                                                 {/* Ramesh Made Changes on 12April24 - Based Type i wil changed default image for customer profile image */}
                                                <img className="mb-2" id="img" src={customerData?.customerPhoto ? customerData?.customerPhoto : USERIMAGE} width="150px" height="150px" style={{ objectFit: "cover" }} />
                                            </div>
                                            <input type="file"
                                                accept="image/*"
                                                name="image-upload"
                                        ...
                                        <div className="row form-outline">
                                            ...
                                            <div className="col-md-12">
                                                <div className="form-row">
                                                    {type!=='CREATE' && <div className="col-md-4">
                                                            ...
                                                            <div className="form-inline">
                                                                {type === 'CREATE' ?
                                                                    <div style={{ width: "30%" }}>
                                                                        <Select
                                                                            id="extn"
                                                                            className={`${(customerError.extn ? "error-border" : "")} `}
                                                                            menuPortalTarget={document.body}
                                                                            styles={{ menuPortal: base => ({ ...base, zIndex: 999999 }) }}
                                                                            options={phoneNoPrefix}
                                                                            isMulti={false}
                                                                            value={[{ label: customerData?.extn, value: customerData?.extn }] || mappedExtn}
                                                                            onChange={(e) => {
                                                                                setMappedExtn(e)
                                                                                let phoneNoLen
                                                                    ...
                                                                    :
                                                                    <input className="form-control form-inline mr-1" style={{ width: "20%" }}
                                                                        type="text" id="code" value={"+" + customerAddress?.countryCode}
                                                                        disabled={type === 'CREATE' ? false : true} />
                                                                }
                                                                <div style={{ width: "70%" }}>
                                                                    <NumberFormatBase className="form-control" id="mobileNo"
                            ...
                            <div className="col-md-12">
                                <div id="customer-buttons" className="skel-btn-center-cmmn">
                                    ...
                                    {/* Ramesh Made Changes on 12April24 - Based Type i wil changed */}
                                   {type!=='CREATE'&& <button type="button" className="skel-btn-submit" onClick={handleDeactivateCustomer}>De-Activate</button>}
                                </div>
                            </div>
                        </div>
                    </div>
