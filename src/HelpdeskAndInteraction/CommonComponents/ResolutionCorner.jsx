...
const ResolutionCorner = (props) => {
  ...
  // console.log('val?.from------------>', val?.from)
  // console.log('val?.from------val?.msg?.conversation?.actionType------>', val?.msg?.conversation?.actionType)
  // console.log('val?.from------val?.msg?.conversation?.description------>', val?.msg?.conversation?.description)
  // console.log('val?.from------val?.msg?.conversation?.type------>', val?.msg?.conversation?.type)
  const {
    setResolutionData,
    workflowApiCall,
    handleAddProducts,
  ...
  const handleFormOnChange = (e, idType, formAttributes, i, fieldType) => {
    if (Array.isArray(e) && e?.length > 0) {
      ...
      if (idType && e?.target?.dataset?.rawData) {
        ...
        if (id === 'month') {
          formAttributes = [formAttributes?.[0]?.fieldSet?.find((ele) => ele?.id === 'month')]
        } else if (id === 'year') {
          formAttributes = [formAttributes?.[0]?.fieldSet?.find((ele) => ele?.id === 'year')]
        }
        setFormDetails(prevDetails => ({
          ...prevDetails,
          [id + '_formAttributes']: [formAttributes],
  ...
  const handleFormSubmit = (e, idx) => {
    ...
    let signatureHasMethods = typeof sigPad?.current == "object" && Object.keys(sigPad?.current)?.length > 0;
    if (e?.target?.id === "signature") {
      // console.log('sigPad?.current======xxx=======', sigPad?.current)
      if (signatureHasMethods && sigPad?.current?.getTrimmedCanvas()?.toDataURL('image/png') === 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAAtJREFUGFdjYAACAAAFAAGq1chRAAAAAElFTkSuQmCC') {
        toast.error('Please Provide Signature')
        return
      }
      unstable_batchedUpdates(() => {
        clickToProceed('FORM_SUBMIT', null, resolutionPayload, idx, {
          ...formDetails, grid: [...values], signature: signatureHasMethods ? (sigPad?.current?.toDataURL('base64') || '') : ''
        });
        sigPad?.current?.off()
        setFormDetails({
          ...formDetails,
          signature: signatureHasMethods ? (sigPad?.current?.toDataURL('base64') || '') : ''
        })
      })
      return
    } else {
      ...
      unstable_batchedUpdates(() => {
        formDetails['signature'] = signatureHasMethods ? (sigPad?.current?.toDataURL('base64') || '') : ''
      ...
      <div className="chat-box clearfix">

        {
          (val?.from === 'user') && val?.msg?.conversation?.actionType === 'SENDMESSAGE' && val?.msg?.conversation?.type === 'string' ? <span>{val?.msg?.conversation?.message}</span>
            : (val?.from === 'bot') && val?.msg?.conversation?.actionType === 'SENDMESSAGE' && val?.msg?.conversation?.type === 'string' ?
              <React.Fragment><div style={{ whiteSpace: 'pre-line' }}><span dangerouslySetInnerHTML={{ __html: displayResponse }} /><span>{!completedTyping && <CursorSVG />}</span></div></React.Fragment>
              : (val?.from === 'user') ?
                <div className="mt-2" id={'hide' + idx}>
                  {
                    Array.isArray(val?.msg?.description) && 'productId' in val?.msg?.description[0] && 'qty' in val?.msg?.description[0] && 'productName' in val?.msg?.description[0] ?
                      ...
                      : <span style={{ whiteSpace: 'pre-wrap' }}>{val?.msg?.description}</span>
                  }
                </div>
                              ...
                              <div className="text-center mt-2" id={'hide' + idx}>
                                {
                                  val?.msg?.conversation?.message?.attributes && val?.msg?.conversation?.message?.attributes?.length > 0 && val?.msg?.conversation?.message?.attributes.map((ele, i) => {
                                    return <div className='radio radio-primary m-2' key={i}>
                                      <input type="radio" id={ele?.id + '_' + i}
                                        name={ele?.id}
                                        onChange={(e) => {
                                          clickToProceed('SELECTED_INTXN', selectedService, resolutionPayload, idx, ele.value)
                                        }}>
                                      </input>
                                                          ...
                                                          : (val?.from === 'bot' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.description === 'COLLECT_PRODUCT_FAMILY') ?
                                                            ...
                                                            : (val?.from === 'bot' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.description === 'SHOW_PRODUCTS') ?
                                                              ...
                                                              : (val?.from === 'user' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.message?.element === 'YES_NO_BUTTON') ?
                                                                <div className="skel-pg-bot-sect-btn" id={'hide' + idx}>
                                                                  {
                                                                    val?.msg?.conversation?.message?.attributes && val?.msg?.conversation?.message?.attributes?.length > 0 && val?.msg?.conversation?.message?.attributes.map((ele, i) => {
                                                                ...
                                                                : (val?.from === 'bot' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.message?.element === 'REQUEST_REPLACE_CHECKBOX') ?
                                                                  ...
                                                                  : (val?.from === 'bot' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.message?.element === 'YES_NO_BUTTON') ?
                                                                    ...
                                                                    : (val?.from === 'bot' && val?.msg?.conversation?.actionType === 'COLLECTINPUT' && val?.msg?.conversation?.type === 'object' && val?.msg?.conversation?.message?.element === 'YES_NO_REDIRECT_BUTTON') ?
                                                                      <div className="skel-pg-bot-sect-btn" id={'hide' + idx}>
                                                                        {
                                                                          val?.msg?.conversation?.message?.attributes && val?.msg?.conversation?.message?.attributes?.length > 0 && val?.msg?.conversation?.message?.attributes.map((ele, i) => {
                                                                            return <button key={i} className="skel-btn-submit" type='button' onClick={(e) => {
