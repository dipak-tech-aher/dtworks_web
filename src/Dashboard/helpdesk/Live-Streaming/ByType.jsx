...
const ByType = (props) => {
    ...
    const getTypeWiseHelpdesk = (searchParams) => {
        axios.post(API_ENDPOINT + properties.HELPDESK_API + '/helpdesk-by-type', { ...searchParams, mode: "LIVE", type: 'LIST' }, {
            headers: {
            ...
            respData?.forEach(item => {
                const createdAtHour = moment(new Date(item?.createdAt)).format('HH:mm');
                const status = !item?.helpdeskTypeDesc?.description ? 'In Queue' : item?.helpdeskTypeDesc?.description;
