...
const DeptWiseMissionTask = (props) => {
    ...
    useEffect(() => {
        ...
        post(properties.LEAD_API + "/dept-wise-open-task-mission", { searchParams: reqBody }).then((response) => {
            if (response?.status == 200) {
                const { data } = response;
                const responseObj = data.map((element) => {
                    let obj = {
                        name: element?.oCurrDeptDesc,
                        code: element?.oCurrDept,
    ...
    const getList = (code) => {
        setCategory(code)
        let reqBody = { ...searchParams, mode: mode, type: "LIST", departmentId: code, limit: perPage, page: currentPage };
...
export default DeptWiseMissionTask;