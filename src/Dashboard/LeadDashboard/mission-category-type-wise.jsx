...
const MissionCategoryTypeWise = (props) => {
    ...
    const [columns, setColums] = useState([
        ...
        },
        {
            ...
            accessor: "oMissionCategoryDesc",
            disableFilters: true,
            id: "oMissionCategoryDesc"
        },
        {
            Header: "Mission type",
            accessor: "oMissionTypeDesc",
            disableFilters: true,
            id: "oMissionTypeDesc"
        },
        {
    ...
    useEffect(() => {
        ...
        post(properties.LEAD_API + "/missions-by-category", { searchParams: reqBody }).then((response) => {
            if (response?.status == 200) {
                const { data } = response;
                const responseObj = data.map((element) => {
                    let obj = {
                        name: element?.oMissionCategoryDesc,
                        code: element?.oMissionCategory,
                        value: Number(element?.oCnt ?? 0),
                        count: Number(element?.oCnt ?? 0),
                        type: element?.oMissionCategoryDesc,
                    }
                    return obj
                });
    ...
    const getList = (code) => {
        setCategory(code)
        let reqBody = { /*...searchParams,*/ mode: mode, type: "LIST", missionCategory: [{ value: code }], limit: perPage, offset: currentPage };
        post(properties.LEAD_API + "/missions-by-category", { searchParams: reqBody }).then((response) => {
            if (response?.status == 200) {
                const { data } = response;
                setDataList([...data]);
    ...
    return (
        <React.Fragment>
            ...
            <Modal size="lg" aria-labelledby="contained-modal-title-vcenter" centered show={modalOpen} onHide={() => {
                ...
                <Modal.Header>
                    <Modal.Title><h5 className="modal-title">Mission List</h5></Modal.Title>
                    <CloseButton onClick={() => {
                        setModalOpen(!modalOpen);
                        setIsRefresh(!isRefresh);
...
export default MissionCategoryTypeWise;