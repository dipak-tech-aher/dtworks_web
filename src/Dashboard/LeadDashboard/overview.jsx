...
const Overview = (props) => {
    ...
    useEffect(() => {
        ...
        post(properties.LEAD_API + "/task-by-status", { searchParams: reqBody }).then((response) => {
            if (response?.status == 200) {
                const { data } = response;
                setTotalLeadsCount(data?.[0]?.oTotalCnt)
    ...
    return (
        <div className="cmmn-skeleton">
            <div className="skel-dashboard-title-base">
                <span className="skel-header-title"> Stakeholder Task Overview </span>
