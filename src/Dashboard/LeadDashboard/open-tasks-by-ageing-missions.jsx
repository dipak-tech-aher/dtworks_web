...
const OpenTasksByAgeingMissions = (props) => {
    ...
    useEffect(() => {
        ...
        post(properties.LEAD_API + "/mission-open-tasks-by-ageing-count", { searchParams: reqBody }).then((response) => {
    ...
    const getList = (code) => {
        ...
        post(properties.LEAD_API + "/mission-open-tasks-by-ageing-list", { searchParams: reqBody }).then((response) => {
            if (response?.status == 200) {
                const { data } = response;
                setDataList([...data]);
...
export default OpenTasksByAgeingMissions;