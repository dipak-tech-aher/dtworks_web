...
const ProductsByOrderValueVsOrderCount = (props) => {
    ...
    const getChartData = async () => {
        try {
            setIsLoading(true)
            let topProductData = {}
            let requestObj = { ...searchParams, filterType: 'TOP_10_PRODUCTS', vCategory2: graphType }
            const response = await post(`${properties.ORDER_API}/oms-dashboard/top-products`, { searchParams: requestObj })
            if (response?.data?.length) {
                // setChartData(response?.data ?? []);
                topProductData['chartData'] = response?.data ?? []
            }

            requestObj = { ...searchParams, filterType: 'Total_order_value', vCategory2: graphType }
            const response1 = await post(`${properties.ORDER_API}/oms-dashboard/top-products`, { searchParams: requestObj })
            if (response1?.data?.length) {
                setTotalOrder(Number(response1?.data[0]?.oTotal ?? 0));
                topProductData['tOrder'] = Number(response1?.data[0]?.oTotal ?? 0)
            }
            ...
            const response2 = await post(`${properties.ORDER_API}/oms-dashboard/top-products`, { searchParams: requestObj })
            if (response2?.data?.length) {
                // setLastMonthOrder(Number(response2?.data[0]?.oTotal ?? 0));
                topProductData['lastMonth'] = Number(response2?.data[0]?.oTotal ?? 0)
            }
            ...
            requestObj = { ...searchParams, filterType: filterType, vCategory2: graphType }
            const response3 = await post(`${properties.ORDER_API}/oms-dashboard/top-products`, { searchParams: requestObj })
            if (response3?.data?.length) {
                // setCurrentMonthOrder(Number(response3?.data[0]?.oTotal ?? 0));
                topProductData['currentMonth'] = Number(response3?.data[0]?.oTotal ?? 0)
            }
            ...
            requestObj = { ...searchParams, filterType: filterType, vCategory2: graphType }
            const response4 = await post(`${properties.ORDER_API}/oms-dashboard/top-products`, { searchParams: requestObj })
            if (response4?.data?.length) {
                // setAverageOrder(Number(response4?.data[0]?.oAvg ?? 0));
                topProductData['avgOrder'] = Number(response4?.data[0]?.oAvg ?? 0)
            }

            setExportData?.('product-order', topProductData);

            let { chartData, tOrder, lastMonth, currentMonth, avgOrder } = topProductData
    ...
    useEffect(() => {
        let { chartData, tOrder, lastMonth, currentMonth, avgOrder } = exportData?.["product-order"] ?? {}
        unstable_batchedUpdates(() => {
            setIsLoading(false)
            if (chartData) setChartData(chartData);
            if (tOrder) setTotalOrder(tOrder);
            ...
            <div className="cmmn-skeleton h-100">
                <div className="skel-dashboard-title-base">
                    <span className="skel-header-title">
                        Top 10 Products by Order Value vs Order Count
                    </span>
                </div>
                ...
                <div className=" w-100 skel-btn-center-cmmn skel-tab-btn-toggle mt-2 mb-2">
                    <div className="btn-group btn-group-tab btn-group-sm" role="group" aria-label="">
                        <button type="button" onClick={() => setGraphType('Order_value')} className={`btn btn-light ${graphType === 'Order_value' ? 'active' : ''}`} id="ordervalue1">
                            Order Value
                        </button>
                        <button type="button" onClick={() => setGraphType('Order_count')} className={`btn btn-light ${graphType === 'Order_count' ? 'active' : ''}`} id="ordercount1">
                            Order Count
                        </button>
                    </div>
                </div>
                        ...
                        <div className="skel-average">
                            ...
                            <div className="skel-average-data">
                                <span className="skel-avg-title">
                                    Average Order {graphType === "Order_value" ? `Value` : `Count`} by Product
                                </span>
                                <span>
                                    per month&nbsp;
                                    {averageOrder >= 0 ?
...
export default ProductsByOrderValueVsOrderCount;