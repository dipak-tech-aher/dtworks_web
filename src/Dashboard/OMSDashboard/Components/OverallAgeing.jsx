...
const OverallAgeing = (props) => {
    ...
    const getChartData = async () => {
        try {
            ...
            setIsLoading(true)
            const response = await post(`${properties.ORDER_API}/oms-dashboard/overall-ageing`, { searchParams: requestObj })
            if (response?.data?.length) {
                setChartData(response?.data ?? []);
                overallData['chartData'] = response?.data ?? []
            }

            setExportData?.('overall-agening', overallData);
        }
        catch (e) {
            console.log('error', e)
        } finally {
    ...
    useEffect(() => {
        let { chartData } = exportData?.["overall-agening"] ?? {}
    ...
    const showDetails = (e, range) => {
        ...
        slowPost(properties.ORDER_API + '/oms-dashboard/overall-ageing', { searchParams: requestObj })
            .then((response) => {
