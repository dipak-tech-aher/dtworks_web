...
export default function ResolutionRatebyChannel() {
  ...
  const getChartData = (entity) => {
    try {
      ...
      setIsLoading(true)
      post(apiDetails[entity].endpoint + '/resolution-rate-by-channel', { searchParams: requestObj })
        .then((response) => {
  ...
  useEffect(() => {
    ...
    if (chartData?.length) {
      ...
      channels = [...new Set(chartData.map(item => item.oChannel))];
      XAixs = [...new Set(chartData.map(item => item.oInterval))];

      // Group data by channel
      const groupedData = groupBy(chartData, 'oChannel');
