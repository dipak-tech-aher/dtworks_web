...
const barColors = ['#00B2BD', '#B5EDF0', '#B5EDF0', "#1f77b4", "#ff7f0e", "#2ca02c", '#54C1EE', '#4E33B0', '#654743', '#184636', 'RoyalBlue', 'green', 'yellow', 'orange', 'purple', 'red', 'grey', 'indigo', 'voilet', 'white', 'Pink', 'Cyan', '#E881F0', '#C876B5', '#E4DB07', '#914C83', '#7C37D3', '#867A5B', '#C29AE1', '#7C8B5B', '#EA4330']
export default function SLA() {
    ...
    const getChartData = (entity) => {
        try {
            ...
            setIsLoading(true)
            post(apiDetails[entity].endpoint + '/sla', { searchParams: requestObj })
                .then((response) => {
                    if (response?.data?.length) SetChartData(response?.data); else SetChartData(response?.data);
                }).catch(error => {
                    console.log(error)
                    SetChartData([])
                }).finally(() => {
