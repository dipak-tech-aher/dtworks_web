...
const TypeWise = memo((props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                slowPost(properties.INTERACTION_API + "/by-type/count", { searchParams, listCountDatatype: 'count' }).then((resp) => {
                    if (resp?.status == 200) {
                        setTypeData(resp?.data?.rows ?? []);
                        const totalCount = resp?.data?.rows?.reduce((total, obj) => total + obj?.oIntxnCount, 0);
                        setTotal(totalCount)
                        let typeWise = {}
                        typeWise['data'] = resp?.data?.rows ?? []
                        typeWise['totalCount'] = totalCount
                        props?.setExportData?.('typeWise', typeWise)
                    }
    ...
    const countClicked = (status) => {
        ...
        slowPost(properties.INTERACTION_API + "/by-type/list", { searchParams: payloadSearchParams, listCountDatatype: 'list' }).then((resp) => {
            if (resp?.status == 200) {
                const data = resp?.data?.rows.sort((a, b) => {
                    return b.oIntxnId - a.oIntxnId;
                });
                setTypeDataList(data ?? []);
                setModalOpen(true)
            }
                    ...
                    <div className="row h-100">
                        <div className="col-12 px-2">
                            <div className="skel-dashboard-title-base">
                                ...
                                <div className="skel-dashboards-icons">
                                    <a href="javascript:void(null)" onClick={(e) => {
                                        e?.stopPropagation();
                                        ...
                                        <tbody>
                                            {typeData?.length > 0 ? (
                                                typeData.map((item, index) => (
                                                    <tr key={index}>
                                                        <td>
                                                            <h5 className="skel-font-sm-bold"> {item.oIntxnTypeDesc ?? 'Others'} </h5>
                                                        </td>
                                                        <td>
                                                            <p className="text-dark mb-0 cursor-pointer txt-underline" onClick={() => countClicked(item.oIntxnTypeCode)}> {item.oIntxnCount} </p>
                                                        </td>
                                                    </tr>
                                                ))
                                            ) : (
                                                <tr>
                                                    <td colSpan={2} className='text-center'>
                                                        <h5 className="font-size-14 mb-0 skel-font-sm-bold">No type wise interactions found</h5>
                                                    </td>
                                                </tr>
                                            )}
                                        </tbody>
                        ...
                        <div className="col-12 px-2 align-self-end">
                            ...
                            <div className="skel-refresh-info">
                                <LastRefreshTime data={{ isRefresh, componentName: 'TypeWise' }} />
                            </div>
                        </div>
                    </div>
                </div>
