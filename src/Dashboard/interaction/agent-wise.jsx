...
const AgentWise = (props) => {
    ...
    useEffect(() => {
        if(apiCall){
            slowPost(properties.INTERACTION_API + "/agent-wise/count", { searchParams }).then((resp) => {
            if (resp?.status == 200) {
                setListData(resp?.data?.rows ?? []);
                const totalCount = resp?.data?.rows?.reduce((total, obj) => total + Number(obj?.oIntxnCount), 0);
                setTotal(totalCount)
                let agentWise={}
                agentWise['data'] = resp?.data?.rows ?? []
                agentWise['totalCount'] = totalCount
                props?.setExportData?.('agentWise', agentWise)
            }
                ...
                <div className="row h-100">
                    <div className="col-12 px-2">
                        <div className="skel-dashboard-title-base">
                            <span className="skel-header-title"> {appConfig?.clientFacingName?.agent ?? "Agent"} wise Interaction ({total})</span>
                                ...
                                <table className="table table-hover mb-0 table-centered table-nowrap">
                                    <tbody>
                                        {listData?.length > 0 ? (
                                            ...
                                            <tr>
                                                <td colSpan={2}>
                                                    <h5 className="font-size-14 mb-0 skel-font-sm-bold">No {appConfig?.clientFacingName?.agent?.toLowerCase() ?? "agent"} wise interactions found</h5>
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
