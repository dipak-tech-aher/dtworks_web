...
const SupportAvailability = (props) => {
    ...
    const { livechatSettings, livechatSettingsError } = props.data
    const { handleOnChange, setLiveChatSettings } = props.handle
    const [liveChatcustomTime, setLiveChatcustomTime] = useState({})

    const time = [
        { value: "00:00", key: "00:00", Description: "12:00 AM" },
    ...
    const handleCustomerAvaliablility = (key) => {
        let livechatCustomAvailability = [...livechatSettings.customAvailability]
         livechatCustomAvailability = livechatCustomAvailability.filter((item) => item.day !== key);
         setLiveChatSettings({
            ...livechatSettings,
            customAvailability: livechatCustomAvailability
        })
    }


    const handleonClick = (e) => {
        const livechatCustomAvailability = []
        if (livechatSettings?.customAvailability.length > 0) {
            livechatSettings?.customAvailability?.forEach((d) => {
                if (d.day === liveChatcustomTime.day) {
                    livechatCustomAvailability.push(liveChatcustomTime)
                } else {
                    livechatCustomAvailability.push(d)
                }
            })
        ...
        if (!(_.some(livechatCustomAvailability, ['day', liveChatcustomTime.day]))) {
            livechatCustomAvailability.push(liveChatcustomTime)
        }
                    ...
                    <div className="col-12">
                        <form action="" method="">
                            <div className="form-group row">
                                <label htmlFor="onlineAvailability" className="col-md-4 col-form-label text-md-left">Always available online</label>
                                <div className="col-md-4">
                                    <input type="checkbox" id="onlineAvailability" className="form-control" checked={livechatSettings.onlineAvailability === 'Y' ? true : false} onChange={handleOnChange} />
                                </div>
                            </div><br>
                            </br>
                        </form>
                        ...
                        <form action="" method="">
                            <div className="form-group row" >
                                <label htmlFor="timeZone" className="col-md-4 col-form-label text-md-left">Select TimeZone</label>
                                <div className="col-md-5">
                                    <select name="timeZone" className={`form-control ${livechatSettingsError.timeZone && "error-border"}`} id="timeZone" value={livechatSettings.timeZone} disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} onChange={handleOnChange} >
                                    ...
                                    <span className="errormsg">{livechatSettingsError.timeZone ? livechatSettingsError.timeZone : ""}</span>
                                </div>
                            </div><br>
                            </br>
                        </form>
                        <form action="" method="">
                            <div id="customAvailability" className="form-group row" >
                                <label htmlFor="customAvailability" className="col-md-4 col-form-label text-md-left">Custom Availability</label>
                                <div className="col-md-2">
                                    <select id="day" className="form-control" disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} onChange={handleOnchange} >
                                ...
                                <div className="col-md-2">
                                    <select className="form-control" id="fromTime" aria-invalid="false" disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} onChange={handleOnchange}  >
                                ...
                                <div className="col-md-2">
                                    <select className="form-control" id="toTime" aria-invalid="false" disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} onChange={handleOnchange}  >
                                ...
                                <div className="col-md-2">
                                    <button type="button" id="addCustomAvailability" disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} onClick={handleonClick} className="text-center btn btn-labeled btn-primary btn-sm">Add</button>
                                </div>
                            </div>
                        </form>
                        <br></br>
                        {livechatSettings.customAvailability.length > 0 && <form action="" method="">
                            <div className="form-group row">
                            <label htmlFor="customAvailability" className="col-md-3 col-form-label text-md-left"></label>
                                <div className="col-md-8">
                                    {livechatSettings.customAvailability && livechatSettings.customAvailability.map((d) => (
                        ...
                        <form>
                            <div className="form-group row" id="offline">
                                <label htmlFor="offlineText" className="col-md-3 col-form-label text-md-left">Description text when offline</label>
                                <div className="col-md-8">
                                    <input type="text" id="offlineText" disabled={livechatSettings.onlineAvailability === 'Y' ? true : ""} name="offlineText" value={livechatSettings.offlineText} onChange={handleOnChange} className={`form-control ${livechatSettingsError.offlineText && "error-border"}`} autoComplete="off" aria-invalid="false" />
                                    <span className="errormsg">{livechatSettingsError.offlineText ? livechatSettingsError.offlineText : ""}</span>
                                </div>
                            </div>
                        </form>
                        <br></br>
