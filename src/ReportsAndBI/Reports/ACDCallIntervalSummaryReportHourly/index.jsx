...
import { ACDCallIntervalSummaryReportHourlyColumns } from "./ACDCallIntervalSummaryReportHourlyColumns";

const ACDCallIntervalSummaryReportHourly = () => {
            ...
            .then((resp) => {
                if (resp && resp.data && resp.data.length > 0) {
                    unstable_batchedUpdates(() => {
                        ...
                        // formatedCampaigns.unshift({ label: 'ALL', value: 'ALL' })
    ...
    const ACDCallIntervalSummaryReportHourly = useCallback((uid) => {
        if (uid) {
            setListSearch({ uid })
            get(`${properties.REPORTS_API}/get-report/acd-call-interval-hourly?limit=${perPage}&page=${currentPage}&uuid=${uid}`)
                .then((resp) => {
    ...
    const getReportDetails = useCallback(() => {
        try {
            ...
            post(`${properties.REPORTS_API}/acd-call-interval-hourly`, { ...requestBody }).then((resp) => {
    ...
    useEffect(() => {
        if (!isFirstRender.current) {
            ACDCallIntervalSummaryReportHourly(uid);
        }
        else {
                    ...
                    <div className="col-12">
                        <section className="triangle">
                            <div className="col-12 row">
                                <div className="col-12"><h4 className="pl-3">ACD Call Interval Summary Report Hourly</h4></div>
                            </div>
                        </section>
                    </div>
                </div>
                                            ...
                                            <div className="col-lg-3 col-md-4 col-sm-12">
                                                <div className="form-group">
                                                    ...
                                                    <select className="form-control" id="interval_start_hours" value={searchInputs?.interval_start_hours}
                                                        onChange={handleInputChange} data-name="Interval Start Hour">
                                                        <option value="">All</option>
                                    ...
                                    <div className="card-body" id="datatable">
                                        <div style={{}}>
                                            <DynamicTable
                                                ...
                                                listKey={"ACD Call Interval Summary Report Hourly"}
                                                row={searchData}
                                                rowCount={totalCount}
                                                header={ACDCallIntervalSummaryReportHourlyColumns}
                                                ...
                                                exportBtn={exportBtn}
                                                report={true}
                                                reportName={"ACD Call Interval Summary Report Hourly"}
                                                url={`${properties.REPORTS_API}/get-report/acd-call-interval-hourly?limit=${perPage}&page=${currentPage}&uuid=${uid}`}
                                                method={'GET'}
                                                handler={{
...
export default ACDCallIntervalSummaryReportHourly;