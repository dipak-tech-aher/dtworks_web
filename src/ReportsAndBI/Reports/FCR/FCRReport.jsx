...
import { FCRColumns } from "./FCRColumns";
...
const FCRRport = () => {
            ...
            .then((resp) => {
                if (resp && resp.data && resp.data.length > 0) {
                    ...
                    unstable_batchedUpdates(() => {
                        ...
                        setDepartmentLookup(formatedData)
                    })
                }
    ...
    const getFCRReport = useCallback((uid) => {
        if (uid) {
            setListSearch({ uid })
            showSpinner()
            axios.get(`${API_ENDPOINT}${properties.REPORTS_API}/get-report/fcr?limit=${perPage}&page=${currentPage}&uuid=${uid}`,
                {
                    headers: {
                        "x-tenant-id": process.env.NODE_ENV === 'development' ? appConfig.biTenantId : undefined,
    ...
    const getReportDetails = useCallback(() => {
        ...
        showSpinner()
        axios.post(`${API_ENDPOINT}${properties.REPORTS_API}/fcr`,
            { ...requestBody },
            { headers: { "x-tenant-id": process.env.NODE_ENV === 'development' ? appConfig.biTenantId : undefined, Authorization: auth?.accessToken } }
        ).then((resp) => {
            const response = resp?.data
                                    ...
                                    <div className="card-body" id="datatable">
                                        <div style={{}}>
                                            <DynamicTable
                                                ...
                                                exportBtn={exportBtn}
                                                report={true}
                                                reportName={"FCR Report"}
                                                url={`${properties.REPORTS_API}/get-report/fcr?limit=${perPage}&page=${currentPage}&uuid=${uid}`}
                                                method={'GET'}
                                                handler={{
