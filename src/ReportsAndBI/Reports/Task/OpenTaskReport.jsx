...
const OpenTaskReport = () => {
    ...
    const handleSubmit = (e) => {
        ...
        post(`${properties.REPORTS_API}/tasks/open`, requestBody)
            .then((resp) => {
                if (resp.data) {
    ...
    const getTaskDetails = () => {
        get(`${properties.REPORTS_API}/get-task/open?limit=${perPage}&page=${currentPage}&uuid=${uuid}`)
            .then((resp) => {
                if (resp.data) {
                    if (resp.status === 200) {
                                                ...
                                                <div className="form-group">
                                                    ...
                                                    >
                                                        ...
                                                        {TaskStatusLookup.map((item) => (
                                                            // Exclude items with codes "hold" and "closed"
                                                            (item.code !== 'TS_HOLD' && item.code !== 'TS_CLOSED') && (
                                                                <option key={item.code} value={item.code}>
                                                                    {item.description}
                                                                </option>
                                                            )
                                    ...
                                    <div className="card-body" id="datatable">
                                        <div style={{}}>
                                            <DynamicTable
                                                ...
                                                exportBtn={exportBtn}
                                                url={`${properties.REPORTS_API}/get-task/open?limit=${perPage}&page=${currentPage}&uuid=${uuid}`}
                                                method={'GET'}
                                                handler={{
