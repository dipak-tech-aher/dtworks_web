...
import { AgentProductivityReportDailyColumns } from "./AgentProductivityReportDailyColumns";

const AgentProductivityReportDaily = () => {
    ...
    const agentProductivityReportDailly = useCallback((uid) => {
        if (uid) {
            setListSearch({ uid })
            get(`${properties.REPORTS_API}/get-report/agent-productivity-daily-report?limit=${perPage}&page=${currentPage}&uuid=${uid}`)
                .then((resp) => {
    ...
    const getReportDetails = useCallback(() => {
        try {
            ...
            post(`${properties.REPORTS_API}/agent-productivity-daily-report`, { ...requestBody }).then((resp) => {
    ...
    useEffect(() => {
        if (!isFirstRender.current) {
            agentProductivityReportDailly(uid);
        }
        else {
                    ...
                    <div className="col-12">
                        <section className="triangle">
                            <div className="col-12 row">
                                <div className="col-12"><h4 className="pl-3">Agent Productivity Daily </h4></div>
                            </div>
                        </section>
                    </div>
                </div>
                                    ...
                                    <div className="card-body" id="datatable">
                                        <div style={{}}>
                                            <DynamicTable
                                                ...
                                                listKey={"Agent Productivity Report Daily"}
                                                row={searchData}
                                                rowCount={totalCount}
                                                header={AgentProductivityReportDailyColumns}
                                                ...
                                                exportBtn={exportBtn}
                                                report={true}
                                                reportName={"Agent Productivity Report Daily"}
                                                url={`${properties.REPORTS_API}/get-report/agent-productivity-daily-report?limit=${perPage}&page=${currentPage}&uuid=${uid}`}
                                                method={'GET'}
                                                handler={{
...
export default AgentProductivityReportDaily;