...
const CreateCustomerInquiryForm = (props) => {
    ...
    const getCustomerDetailsByServiceNo = (data) => {
        ...
        ameyoPost(properties.CUSTOMER_API + "/search?limit=10&page=0", requestParam, accessToken)
            .then((resp) => {
                if (resp.data) {
                    if (resp.status === 200) {
                        ...
                        if (apiData) {
                            ...
                            ameyoGet(`${properties.CUSTOMER_DETAILS}/${apiData.customerId}?serviceId=${apiData.serviceId}`, '', accessToken)
                                .then((customerResp) => {
