...
const ReviewContract = (props) => {
    ...
    const { viewContractList, viewContractInputs, viewContractCounts, viewContractTotalRowCount, viewContractPerPage, viewContractCurrentPage, viewContractRegenerateCount, billingReadOnly, listSearch, listLimit } = props.data;
    const { setViewContractList, handleOnViewContractsInputsChange, handleOnViewContractSearch, handleViewContractPageSelect, setViewContractPerPage, setViewContractCurrentPage, doSoftRefreshForContract, handleOnViewContractsInputsClear, setListLimit } = props.handler;
    ...
    const handleCellRender = (cell, row) => {
        if (cell.column.Header === "Contract ID") {
            return (<span className="text-secondary cursor-pointer" id="CUSTOMERID" onClick={(e) => handleOnViewContract(row.original)}>{cell.value}</span>)
        }
        ...
        // }
            ...
            return (
                <button type="button" className="btn btn-labeled btn-primary btn-sm ml-1"
            ...
            return (
                <button type="button" className={`btn btn-labeled btn-primary btn-sm ml-1 ${isReview ? '' : 'd-none'}`}
    ...
    const handleOnHoldUnholdContract = (contract) => {
        ...
        console.log("View Contract Current Page =>", viewContractCurrentPage);
        ...
        unstable_batchedUpdates(() => {
            put(properties.CONTRACT_API + '/unbilled/' + contract.monthlyContractId, {
                ...
                .then((resp) => { doSoftRefreshForContract(listSearch, viewContractCurrentPage, listLimit) })
                .finally()
        })
    }
    ...
    const handleHoldAll = () => {
        const holdCount = viewContractList.filter((x) => x?.isOnhold === 'N')?.length
        const ids = viewContractList.filter((x) => x?.isOnhold === 'N')?.map((y) => y.monthlyContractId)
        if (holdCount === 0) {
            toast.error("No Contracts Availalble for Hold")
            return
        } else {
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to Hold " + holdCount + " Contract",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    unstable_batchedUpdates(() => {
                        put(properties.CONTRACT_API + '/unbilled/' + ids, {
                            status: 'HOLD'
                        })
                            .then((resp) => { doSoftRefreshForContract(listSearch) })
                            .finally()
                    })
                }
            })
    ...
    const handleUnholdAll = () => {
        const unholdCount = viewContractList.filter((x) => x?.isOnhold === 'Y')?.length
        const ids = viewContractList.filter((x) => x?.isOnhold === 'Y')?.map((y) => y.monthlyContractId)
        if (unholdCount === 0) {
            toast.error("No Contracts Availalble for Unhold")
            return
        } else {
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to Unhold " + unholdCount + " Contract",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
            ...
            <div className="col-12">
                ...
                <section>
                    ...
                    <div className="border">
                        <button type="button" className={`btn btn-labeled btn-primary btn-sm m-1 ${isReview ? '' : 'd-none'}`} onClick={() => handleHoldAll()}>
                            <small>
                                Hold All
                            </small>
                        </button>
                        <button type="button" className={`btn btn-labeled btn-primary btn-sm m-1 ${isReview ? '' : 'd-none'}`} onClick={() => handleUnholdAll()}>
                            <small>
                                Unhold All
                            </small>
                        </button>
